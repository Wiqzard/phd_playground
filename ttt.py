...
if __name__ == "__main__":
    ...
    ##################################
    # 1. Dummy Dataset and DataLoader
    ##################################
    class DummyTextDataset(Dataset):
        """
        A simple dataset that returns random token IDs and (optionally) labels.
        For real use, you'd load your actual text data, tokenize it, etc.
        """

        def __init__(self, num_samples=1000, seq_length=32, vocab_size=32000):
            super().__init__()
        ...
        def __getitem__(self, idx):
            ...
            input_ids = torch.randint(
                low=0, high=self.vocab_size, size=(self.seq_length,), dtype=torch.long
            )
            # For language modeling, labels are typically the same as input_ids shifted
    ...
    def collate_fn(batch):
        """
        Simple collate_fn to combine samples into a batch.
        """
    ...
    # 2. Initialize Config
    ########################
    # Example: Choose from standard configs or create your own
    config = TTTConfig(
        vocab_size=32000,
        hidden_size=768,
        intermediate_size=2048,
        num_hidden_layers=12,
        num_attention_heads=12,
        ttt_layer_type="linear",  # can also be "mlp"
    ...
    dataset = DummyTextDataset(
        num_samples=200, seq_length=32, vocab_size=config.vocab_size
    )
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(dataloader):
            ...
            outputs = model(
                input_ids=input_ids,
                labels=labels,
                use_cache=False,  # turn off caching for typical training
            )
            loss = outputs.loss
    ...
    # 5. Save the Model
    ######################
    model.save_pretrained("ttt-model-checkpoint")
    print("Model saved to ttt-model-checkpoint!")
