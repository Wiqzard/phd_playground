...
class TTTCache:
    ...
    def __init__(self, model, batch_size: int, device=None):
        ...
        iterator = (
            model.memory_layer_indices
            if hasattr(model, "memory_layer_indices")
    ...
    def update(self, py_tree, layer_idx, seq_len):
        if seq_len % self.mini_batch_size == 0:
            ...
            for name in self.ttt_param_names:
                new_states = py_tree[
                    f"{name}_states"
                ]  # .clone() #.detach()  # or clone() if needed
    ...
    def __init__(self, config: TTTConfig, layer_idx: Optional[int] = None):
        ...
        self.W = nn.Parameter(
            torch.normal(
                0,
                0.02,
                size=(num_experts, self.num_heads, self.head_dim, self.head_dim),
            )
        )
        self.b = nn.Parameter(
            torch.zeros(num_experts, self.num_heads, 1, self.head_dim)
        )
    ...
    def ttt(
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            if False:  # self.config.output_ttt_stats:
                ...
                Z2_ln_init = ln_fwd(
                    Z2, ln_weight, ln_bias
                )  # LN of Z2 from current mini-batch params
                diff_step_0 = (
                    Z2_ln_init[:, :, -1, :] - reconstruction_target[:, :, -1, :]
                )
                ...
                if True:
                    with torch.no_grad():
                        ...
                        diff_init = (
                            Z2_0_ln[:, :, -1, :] - reconstruction_target[:, :, -1, :]
                        )
            ...
            return (
                last_param_dict,
                XQW_mini_batch,
            )  # , ttt_loss_mse_init, ttt_loss_mse_step_0, ttt_loss_mse_step_1)
