...
class TTTConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.output_ttt_stats = output_ttt_stats
...
class TTTCache:
    ...
    def __init__(self, model, batch_size: int, device=None):
        ...
        iterator = model.memory_layer_indices if hasattr(model, "memory_layer_indices") else range(config.num_hidden_layers)
        for layer_idx in iterator:
            for name in self.ttt_param_names:
                weight = getattr(model.layers[layer_idx].seq_modeling_block, name)
                tiled_weight = torch.tile(weight.unsqueeze(0), (batch_size,) + (1,) * weight.dim()).to(self.device)
    ...
    def update(self, py_tree, layer_idx, seq_len):
        if seq_len % self.mini_batch_size == 0:
            ...
            for name in self.ttt_param_names:
                new_states = py_tree[f"{name}_states"] #.clone() #.detach()  # or clone() if needed
                new_grads = torch.zeros_like(new_states)
                ## Instead of copy_() or zero_(), just store them:
                self.ttt_params_dict[f"{name}_states"][layer_idx] = new_states
                self.ttt_params_dict[f"{name}_grad"][layer_idx] = new_grads
                #print("here")
...
class TTTMoELinear(TTTBase):
    def __init__(self, config: TTTConfig, layer_idx: Optional[int] = None):
        ...
        num_experts = 8
        self.W = nn.Parameter(torch.normal(0, 0.02, size=(num_experts, self.num_heads, self.head_dim, self.head_dim)))
        self.b = nn.Parameter(torch.zeros(num_experts, self.num_heads, 1, self.head_dim))
    ...
    def ttt(
        ...
        def compute_mini_batch(params_dict, inputs):
            ...
            if use_dual_form:
                ...
                #b1_bar = b1_init - torch.tril(eta_mini_batch) @ grad_l_wrt_Z1
                b1_bar = (b1_init - torch.tril(eta_mini_batch) @ grad_l_wrt_Z1) ######.clone()
                ...
                b2_bar = (b2_init - torch.tril(eta_mini_batch) @ grad_l_wrt_Z2) #######.clone()
                ...
                #W1_last = W1_init - (last_eta_mini_batch * X1).transpose(-1, -2) @ grad_l_wrt_Z1
                W1_last = (W1_init - (last_eta_mini_batch * X1).transpose(-1, -2) @ grad_l_wrt_Z1) #######.clone()
                ...
                W2_last = (W2_init - (last_eta_mini_batch * X2).transpose(-1, -2) @ grad_l_wrt_Z2) #######.clone()
                # [B,nh,1,f]
            ...
            if False: #self.config.output_ttt_stats:
                #
                # 1) ttt_loss_mse_step_0
                #
                # "Step 0" = MSE of the last token *before* final update of the last token
                # In the JAX code, that was (ttt_norm_out[-1] - ssl_target[-1])**2. Here:
                Z2_ln_init = ln_fwd(Z2, ln_weight, ln_bias)  # LN of Z2 from current mini-batch params
                diff_step_0 = Z2_ln_init[:, :, -1, :] - reconstruction_target[:, :, -1, :]
                ttt_loss_mse_step_0 = (diff_step_0**2).mean()

                #
                # 2) ttt_loss_mse_init
                #
                # In the JAX code, this used the *original* full-sequence parameters (W1_0, W2_0, ...)
                # If you have them in something like self.full_params_dict, do another forward pass:
                # For illustration, assume you have original W1_0, etc. stored in `self.full_init`.
                # If you do *not* have them, you can omit or set None.
                if True:
                    with torch.no_grad():
                        W1_0 = self.W1
                        b1_0 = self.b1
                        ...
                        Z1_0 = X1 @ W1_0 + b1_0
                        X2_0 = F.gelu(Z1_0, approximate="tanh")
                        Z2_0 = X2_0 @ W2_0 + b2_0
                        Z2_0_ln = ln_fwd(Z2_0, ln_weight, ln_bias)
                        diff_init = Z2_0_ln[:, :, -1, :] - reconstruction_target[:, :, -1, :]
                        ttt_loss_mse_init = (diff_init**2).mean()
                else:
                    ttt_loss_mse_init = None

                # 3) ttt_loss_mse_step_1
                # "Step 1" = MSE after updating last-token parameters (W1_last, W2_last, b1_last, b2_last).
                # Do forward pass on only the last token X1[:, :, -1, :].
                # "Last token" in shape [B, nh, 1, f]
                X1_last = X1[:, :, -1:, :]          # shape [B, nh, 1, f]

                # W1_last is [B, nh, f, 4f], b1_last is [B, nh, 1, 4f], etc.
                Z1_last = (X1_last @ W1_last) + b1_last     # [B, nh, 1, 4f]

                X2_last = F.gelu(Z1_last, approximate="tanh")  # [B, nh, 1, 4f]
                Z2_last = (X2_last @ W2_last) + b2_last        # [B, nh, 1, f]

                # Then apply LN
                Z2_last_ln = ln_fwd(Z2_last, ln_weight, ln_bias)  # [B, nh, 1, f]

                # Finally, compare to the "last token" of your reconstruction target
                diff_step_1 = Z2_last_ln - reconstruction_target[:, :, -1:, :]
                ttt_loss_mse_step_1 = (diff_step_1 ** 2).mean()
            ...
            return last_param_dict, XQW_mini_batch #, ttt_loss_mse_init, ttt_loss_mse_step_0, ttt_loss_mse_step_1)
