...
from titans.titans_pytorch.associative_scan import AssocScan

from titans.titans_pytorch.memory_models import MemoryMLP, ResidualNorm
...
def slice_dict_values(td, dim, start, end):
    length = end - start
    return td.apply(lambda t: t.narrow(dim, start, length))
...
class NeuralMemory(Module):
    def __init__(
        ...
        default_step_transform_max_lr=1.0,
        per_parameter_lr_modulation=False,  # allow outer network to control learning rate per weight matrix of memory network
        max_mem_layer_modulation=1.0,  # max of 10.
        ...
        with torch.no_grad():
            ...
            ), "output of memory model needs to be same shape as input"
        ...
        if per_head_learned_parameters:
            memory_model_parameters = [
                repeat(p, "... -> h ...", h=heads) for p in memory_model_parameters
            ]
        ...
        self.to_learned_weight_residual_mix = (
            Sequential(nn.Linear(dim, heads), Rearrange("b n h -> b h n"), nn.Sigmoid())
        ...
        self.transition_gate = nn.Parameter(tensor(-5.0)) if gated_transition else None
    ...
    def init_weights(
        ...
        if self.per_head_learned_parameters:
            weights = repeat_dict_values(
                self.memory_model_parameter_dict, "h ... -> (b h) ...", b=batch
            )
        else:
            weights = repeat_dict_values(
                self.memory_model_parameter_dict, "... -> bh ...", bh=batch * self.heads
            )
    ...
    def init_momentum(
        ...
        if self.per_head_learned_parameters:
            zeros = repeat_dict_values(
                zeros, "h ... -> o (b h) ...", b=batch, o=self.momentum_order
            )
        else:
            zeros = repeat_dict_values(
                zeros, "... -> o bh ...", bh=batch * self.heads, o=self.momentum_order
            )
    ...
    def store_memories(
        ...
        unweighted_mem_model_loss = rearrange(
            unweighted_mem_model_loss, "(b h n) c -> b h (n c)", b=batch, h=heads
        )
        ...
        if num_chunks == 0:
            ...
            next_store_state = NeuralMemState(
                next_seq_len_index, weights, remainder, past_state, updates
            )
        ...
        for (param_name, surprise), (_, last_update) in zip(
            ...
            if has_momentum:
                ...
                momentums = (
                    []
                )  # stores all momentum orders starting with first, to generalize to Nth order momentum
                ...
                for one_adaptive_momentum, one_last_momentum in zip_longest(
                    ...
                    momentum = self.assoc_scan(
                        one_adaptive_momentum, momentum, prev=one_last_momentum
                ...
                next_last_momentum[param_name] = momentums[
                    :, :, -1
                ]  # momentums shape is Float['o bh n 1']
            ...
            update = self.assoc_scan(
                1.0 - decay_factor, update, prev=last_update, remove_prev=False
            )
        ...
        next_store_state = NeuralMemState(
            next_seq_len_index, weights, remainder, next_state, updates
        )
    ...
    def forward(
        ...
        seq_index, weights, cache_store_seq, past_state, _ = state
                ...
                last_update = TensorDict(
                    {
                        ...
                        for (param_name, one_weight), (_, one_last_update) in zip(
                            weights.items(), last_update.items()
                        )
                    }
                )
...
class Gate(nn.Module):
    ...
    def __init__(
        ...
        self.n_groups = n_expert_groups  # optional
        self.topk_groups = n_limited_groups  # optional
        self.score_func = score_func
        self.route_scale = route_scale
        self.weight = nn.Parameter(torch.empty(n_routed_experts, dim))
        self.bias = (
            nn.Parameter(torch.empty(n_routed_experts)) if self.dim == 7168 else None
        )
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        scores = F.linear(x, self.weight)  # linear(x, self.weight)
...
class NeuralMemoryMOE(Module):
    def __init__(
        ...
        #        test_input = torch.randn(test_shape)
        #        if moe:
        #            routing_weights, indices = torch.randn(3, 2, heads), torch.randint(0, 8, (3, 2))
        #            mem_model_output = model(test_input, routing_weights, indices)
        #        else:
        #            mem_model_output = model(test_input)
        #    except:
        ...
        if self.moe:
            ...
            def forward_and_loss(
                params, inputs, loss_weights, target, routing_weights, routing_indices
            ):
                inputs = (inputs, routing_weights, routing_indices)
                pred = functional_call(self.memory_model, params, inputs)
                loss = self.store_memory_loss_fn(
                    pred, target
                )  # simple mse loss in paper - eq (12) - |M(k) - v|²
        ...
        if self.moe:
            # self.per_sample_grad_fn = vmap(grad_fn, in_dims = (0, 0, 0, 0, 0, 0))
            self.per_sample_grad_fn = grad_fn
        else:
            self.per_sample_grad_fn = vmap(grad_fn, in_dims=(0, 0, 0, 0))
    ...
    def store_memories(
        ...
        if self.moe:
            routing_weights, indices = self.router(seq)
            grads, unweighted_mem_model_loss = self.per_sample_grad_fn(
    ...
    def retrieve_memories(
        ...
        if self.moe:
            routing_weights, indices = self.router(seq)
            routing_weights = rearrange(
                routing_weights, "b (n c) h -> (b n) c h", c=chunk_size
            )
            indices = rearrange(indices, "b (n c) d -> (b n) c d", c=chunk_size)
            inputs = (queries, routing_weights, indices)
            values = functional_call(
                self.memory_model, dict(weights), inputs
            )  # queries, routing_weights, indices)
        else:
            values = functional_call(self.memory_model, dict(weights), queries)
    ...
    def forward(
        ...
        if True:
            retrieve_updates = slice_dict_values(updates, 1, 0, len(store_seqs))
            retrieved = self.retrieve_memories(retrieve_seq, retrieve_updates)
        else:
            retrieved = self.retrieve_memories(retrieve_seq, updates)
