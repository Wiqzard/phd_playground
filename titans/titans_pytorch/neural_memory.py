...
from titans.titans_pytorch.memory_models import(
    MemoryMLP,
    ResidualNorm
)
...
class NeuralMemoryLeg(Module):
    ...
    def forward(
        ...
        store_seq_len, _, batch_size = store_seq.shape[-2], self.chunk_size, self.batch_size
...
class Gate(nn.Module):
    ...
    """
    def __init__(self, dim, n_activated_experts, n_expert_groups, n_limited_groups, n_routed_experts, score_func, route_scale):
        """
...
class NeuralMemory(Module):
    def __init__(
        ...
        if self.moe:
            def forward_and_loss(params, inputs, loss_weights, target, routing_weights, routing_indices):
        ...
        if moe:
            n_experts = 8
            top_k = 1
            self.router = Gate(dim, n_routed_experts=n_experts, n_activated_experts=top_k, n_expert_groups=0,
                                n_limited_groups=0,
    ...
    def store_memories(
        ...
        if self.moe:
            ...
            grads, unweighted_mem_model_loss = self.per_sample_grad_fn(dict(weights_for_surprise), keys, adaptive_lr, values, routing_weights, indices)
        else:
            grads, unweighted_mem_model_loss = self.per_sample_grad_fn(dict(weights_for_surprise), keys, adaptive_lr, values)
...
#        seq = pad_at_dim(seq, (0, padding), dim = 1)
        ...
        if self.moe:
            routing_weights, indices = self.router(seq)
            routing_weights = rearrange(routing_weights, 'b (n c) h -> (b n) c h', c = chunk_size)
            indices = rearrange(indices, 'b (n c) d -> (b n) c d', c = chunk_size)
            inputs = (queries, routing_weights, indices)
            values = functional_call(self.memory_model, dict(weights), inputs) #queries, routing_weights, indices)
        else:
            values = functional_call(self.memory_model, dict(weights), queries)
