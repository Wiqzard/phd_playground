...
class SegmentedAttention(Module):
    def __init__(
        ...
        self.attend = Attend(causal=False, **attend_kwargs)
        ...
        for layer in layers:
            ...
            if layer in neural_memory_layers:
                ...
                if not is_first and neural_memory_qkv_receives_diff_views:
                    num_layer_choices = (
                        layer - 1
                    ) * 4 + 1  # for each layer, have memory input select from attn inp, attn out, ff inp, and ff out - plus one for the current point in the residual stream (memory input)
                ...
                mem = NeuralMemory(
                    ...
                    accept_weight_residual=neural_mem_weight_residual
            ...
            if exists(mem):
                ...
                if not exists(mem_qkv_layer_selector):
                    ...
                    qkv_mem_input = einsum(
                        layers_to_choose_from, selected, "l b n d, v b n l -> v b n d"
                    )
