...
@pytest.mark.parametrize(
    "chunk_size, attn_pool_chunks", ((64, True), (64, False), (1, False))
)
@pytest.mark.parametrize("momentum", (False, True))
@pytest.mark.parametrize("qk_rmsnorm", (False, True))
@pytest.mark.parametrize("heads", (1, 4))
@pytest.mark.parametrize("max_grad_norm", (None, 2.0))
...
def test_titans_second_order_momentum(
    learned_momentum_combine, learned_combine_include_zeroth
):
...
def test_titans_attn_memory():
    ...
    mem = NeuralMemory(dim=16, chunk_size=64, model=MemoryAttention(dim=16))
...
def test_neural_mem_chaining_chunks(gated_transition):
    mem = NeuralMemory(
        dim=16, dim_head=16, heads=2, chunk_size=16, gated_transition=gated_transition
    )
    ...
    assert torch.allclose(
        parallel_retrieved,
        torch.cat((first_retrieved, second_retrieved, third_retrieved), dim=1),
        atol=1e-5,
    )


def test_neural_mem_chaining_with_weight_residual():
    mem = NeuralMemory(dim=16, dim_head=16, heads=2, chunk_size=64)

    mem2 = NeuralMemory(
        dim=16, dim_head=16, heads=2, chunk_size=64, accept_weight_residual=True
    )
    ...
    second_retrieved, state2 = mem2(
        seq_second, state=state1, prev_weights=state.updates
    )

    assert torch.allclose(
        parallel_retrieved,
        torch.cat((first_retrieved, second_retrieved), dim=1),
        atol=1e-5,
    )


def test_neural_mem_chaining_with_batch_size():
    mem = NeuralMemory(dim=16, dim_head=16, heads=2, chunk_size=16, batch_size=64)
    ...
    parallel_part_retrieved = torch.cat(
        (first_retrieved, second_retrieved, third_retrieved), dim=1
    )
...
def test_mac(
    ...
    transformer = MemoryAsContextTransformer(
        ...
        neural_memory_kwargs=dict(momentum=neural_mem_momentum),
    )
...
def test_mac_sampling(sliding, mem_layers, longterm_mems, prompt_len):
    transformer = MemoryAsContextTransformer(
        num_tokens=256,
        dim=16,
        depth=4,
    ...
    sampled = transformer.sample(prompt, 53, use_cache=False, temperature=0.0)
    sampled_with_cache = transformer.sample(prompt, 53, use_cache=True, temperature=0.0)
...
@torch_default_dtype(torch.float64)
def test_neural_mem_inference(seq_len, prompt_len, mem_chunk_size, gated_transition):

    mem = NeuralMemory(
        dim=16, chunk_size=mem_chunk_size, gated_transition=gated_transition
    )
...
def test_assoc_scan(use_accelerated):
