...
from src.models.components.moe_lora import (
    inject_lora,
    disable_all_adapters,
    enable_all_adapters,
    set_lora_trainability,
    reset_all_lora_parameters,
    get_lora_adapter_parameters,
    get_lora_adapter_parameters,
    set_global_trainability,
...
class DiffusionModelTrainer(LightningModule):
    ...
    def training_step(
        ...
        if self.hparams.meta_learning:
            outer_loss = self.meta_learning_step(batch, batch_idx)
            return outer_loss
        else:
            ...
            self.log(
                "train/loss", total_loss, on_step=True, on_epoch=False, prog_bar=True
            )
            self.log(
                "train/recon_loss",
                recon_loss,
    ...
    def setup(self, stage: str) -> None:
        ...
        if self.hparams.meta_learning:
            inject_lora(
                self.model,
                num_lora_slots=self.hparams.batch_size,
