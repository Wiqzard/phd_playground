...
from utils.torch_utils import spatial_transform, vis_torch_image
from utils.roi_utils import RectangleRegion
...
class DINOFeaturesProvider:
    def __init__(self,
        ...
        self.patch_size = 28
        self.max_condition_tokens = len(condition_frames) * int(cage_crop.min_w * cage_crop.min_h * float((image_width // self.patch_size) * (image_height // self.patch_size)))
        assert num_condition_tokens >= 1 and num_condition_tokens <= self.max_condition_tokens, f"num_condition_tokens should be in [1, {self.max_condition_tokens}]"

        assert (image_width // 14) % 2 == 0 and (image_height // 14) % 2 == 0, "Image width and height should be divisible by 2 after dividing by 14"
        ...
        self.num_pos_emb_tokens = self.dino_patches_width * self.dino_patches_height * len(condition_frames)
    ...
    def get_features(self, target_frames: torch.Tensor, force_roi: RectangleRegion = None, force_condition_frames: torch.Tensor = None) -> torch.Tensor:
        """
        ...
        """
        frames = self.condition_frames if force_condition_frames is None else force_condition_frames
    ...
    def get_patches_to_select(self, cage_feats: torch.Tensor) -> torch.Tensor:
        ...
        for b in range(B):
            ...
            perm = torch.randperm(patches_to_select.size(0), device=patches_to_select.device)[:self.num_condition_tokens]
    ...
    def pick_patches(self, dino_feats: torch.Tensor, patches_to_pick: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
    ...
    def create_random_mask(self, cond_feats: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: [b T 1]
        """
        return torch.bernoulli(torch.full((cond_feats.size(0), cond_feats.size(1), 1), 1 - self.token_dropout_prob, device=cond_feats.device))
...
def create_crop_region(min_w: float = 0.3,
                       min_h: float = 0.3,
                       max_w: float = 0.8,
                       max_h: float = 0.5,
                       y_limit: float = 0.0,):
...
def crop_and_get_dino_features(num_dino_layers: int,
                               dino: nn.Module,
                               target_frames: torch.Tensor,
                               cage_crop: DictConfig,
                               force_roi: RectangleRegion = None) -> torch.Tensor:
    """
    ...
    z_where = create_crop_region(**cage_crop).to(dtype).to(target_frames.device) if force_roi is None else torch.tensor([
        [force_roi.w, force_roi.h, force_roi.x, force_roi.y]
    ], dtype=dtype, device=target_frames.device)
