...
class MemoryEfficientCrossAttention(nn.Module):  # we are using this implementation
    ...
    def forward(
        ...
        if self.add_lora:
            return (
                self.to_out(out)
                + self.out_adapter_up(self.out_adapter_down(out)) * self.lora_scale
            )
        else:
            return self.to_out(out)
...
class SpatialTransformer(nn.Module):
    """Transformer block for image-like data. First, project the input (aka embedding) and reshape
    to b, t, d. Then apply standard transformer action. Finally, reshape to image.
