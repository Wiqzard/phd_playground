...
class FlowNetwork(nn.Module):
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        folded_large_observations = rearrange(
            flat_large_observations, "(b n) c h w -> b n c h w", n=num_observations
        )

        # Extract image pairs
        flat_first_images = rearrange(
            folded_large_observations[:, :-1], "b n c h w -> (b n) c h w"
        )
        flat_second_images = rearrange(
            folded_large_observations[:, 1:], "b n c h w -> (b n) c h w"
        )
