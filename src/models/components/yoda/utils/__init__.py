...
class TemporalDownSample(nn.Module):
    ...
    def __init__(
        ...
        downsample_context: bool = False,
        downsample_residuals: bool = False,
    ):
        super().__init__()
        self.in_channels = in_channels
        out_channels = in_channels if out_channels is None else out_channels
        ...
        self.downsample_context = downsample_context
        self.downsample_residuals = downsample_residuals
        ...
        if num_in_frames != num_out_frames:
            kernel_size = (num_in_frames - num_out_frames + 1, 1, 1)
            padding = [0, 0, 0]
        ...
        if self.use_in_shortcut:
            self.conv_shortcut = nn.Conv3d(
                ...
                kernel_size=(
                    1
                    if num_in_frames == num_out_frames
                    else [num_in_frames - num_out_frames + 1, 1, 1]
                ),
                stride=1,
                padding=0,
            )
        ...
        if self.downsample_context:
            # from bs, in_frames, token, dim -> bs, out_frames, token, dim
            self.context_downsample = nn.Conv1d(
                num_in_frames,
                num_out_frames,
                kernel_size=1,
                stride=1,
    ...
    def forward(
        ...
        bf, c, h, w = input_tensor.shape
        bs = bf // self.num_in_frames
        # input_tensor = batch * frames, channels, height, width
        # want batch, channels, frames, height, width
        ...
        if self.time_emb_proj is not None:
            temb = temb.reshape(-1, self.num_in_frames, temb.shape[-1])
            temb_out = temb
            temb = self.nonlinearity(temb)
            temb = self.time_emb_proj(temb)[:, :, :, None, None]
        ...
        output_tensor = output_tensor.permute(0, 2, 1, 3, 4).reshape(
            bs * self.num_out_frames, c, h, w
        )

        return output_tensor, residuals, temb, encoder_hidden_states
    ...
    def forward(
        ...
        for resnet in self.resnets:
            if self.training and self.gradient_checkpointing:
                ...
                if False:  # is_torch_version(">=", "1.11.0"):
                    hidden_states = torch.utils.checkpoint.checkpoint(
        ...
        for i, resnet in enumerate(self.resnets):
            ...
            if len(self.temporal_downsample) == 1:
                if self.in_frames[i] != self.out_frames:
                    res_hidden_states = self.temporal_downsample[0](res_hidden_states)[
                        0
                    ]
