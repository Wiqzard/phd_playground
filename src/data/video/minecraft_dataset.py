    ...
    def setup(self, stage=None):
        if stage in (None, "fit"):
            self.train_dataset = MinecraftVideoDataset(
                self.save_dir, self.resolution, self.external_cond_dim, self.n_frames, self.frame_skip, self.validation_multiplier, "training"
            )
...
if __name__ == "__main__":
    data_module = MinecraftDataModule(
        save_dir="/data/cvg/sebastian/minecraft_df",
        resolution=64,
        external_cond_dim=0,
        n_frames=64,
        frame_skip=2,
