...
class NeuralMemory2(Module):
    ...
    #        #weights_for_surprise = rearrange_dict_values(weights, '(b 1) ... -> b 1 ...', b = batch)
    ...
    #        # get grads and extra auxiliary loss (for backwarding through qkv projection in base neural memory module)
    #
    #        grads, unweighted_mem_model_loss = self.grad_fn(dict(weights_for_surprise), keys, adaptive_lr, values)
    ...
    def forward(
        ...
        # )
        retrieved = self.retrieve_memories(
            retrieve_seq, rearrange_dict_values(updates, "b ... -> b 1 ...")
        )
...
if __name__ == "__main__":
    from titans.titans_pytorch.memory_models import MemoryMLP, MemoryMoE
    ...
    retrieved, next_neural_mem_state = neural_memory(
        test_seq + retrieved, state=next_neural_mem_state
    )
    loss = torch.nn.functional.mse_loss(retrieved, test_seq)
    loss.backward()
    print(loss)
    print(0)
