...
class RLLightningModule(pl.LightningModule):
    def __init__(self,
        ...
        self.reward_end_model = RewardEndModel(self.obs_dim + self.action_dim, hidden_dim)
    ...
    def update_diffusion_model(self):
        ...
        target = torch.tensor(seq[-1][4], dtype=torch.float32, device=self.device).unsqueeze(0)
        # Sample a noise level sigma using a log-normal parameterization.
        log_sigma = torch.randn(1, device=self.device) * self.hparams.P_std + self.hparams.P_mean
        sigma = log_sigma.exp()
    ...
    def update_reward_end_model(self):
        ...
        reward_targets = torch.tensor(reward_targets, dtype=torch.long, device=self.device).unsqueeze(0)
        done_targets = torch.tensor(done_targets, dtype=torch.long, device=self.device).unsqueeze(0)
        
        reward_logits, done_logits, _ = self.reward_end_model(inputs)
        loss_reward = F.cross_entropy(reward_logits.view(-1, reward_logits.size(-1)),
                                      reward_targets.view(-1))
        loss_done = F.cross_entropy(done_logits.view(-1, done_logits.size(-1)),
                                    done_targets.view(-1))
    ...
    def update_actor_critic(self):
        ...
        current_obs = torch.tensor(seq[-1][4], dtype=torch.float32, device=self.device).unsqueeze(0)
        ...
        for i in range(self.hparams.H):
            ...
            r_inp = torch.cat([current_obs, action_onehot], dim=-1).unsqueeze(0)  # (1,1,input_dim)
    ...
    def configure_optimizers(self):
        optimizer_diffusion = torch.optim.Adam(self.diffusion_model.parameters(), lr=self.hparams.lr_diffusion)
        optimizer_reward = torch.optim.Adam(self.reward_end_model.parameters(), lr=self.hparams.lr_reward)
